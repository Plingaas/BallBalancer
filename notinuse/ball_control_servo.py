...
port_id = '/dev/cu.usbserial-DA00UGWW'
...
def servo_control(key2, queue):
    ...
    def writeCoord():
        ...
        if corrd_info == 'nil': # Checks if the output is nil
            ...
            # PID
            dx = corrd_info[0]
            reg_x = PIDX(target[0], corrd_info[0], dt)
            new_x = dx + reg_x

            dy = corrd_info[1]
            reg_y = PIDY(target[1], corrd_info[1], dt)
            new_y = dy + reg_y
            
            corrd_info = getAngles(-np.deg2rad(new_y), np.deg2rad(new_x))
            print(corrd_info)
